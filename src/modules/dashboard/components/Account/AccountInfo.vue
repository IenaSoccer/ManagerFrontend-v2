  ...
  <div v-if="!isLoading && account"
    ...
    <div class="flex items-center justify-between mb-6">
      ...
      <div class="flex items-center gap-2">
        <button @click="toggleEditMode" class="p-2 text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg transition"
        ...
        <button @click="logout" :disabled="isLogoutButtonDisabled"
          class="p-2 text-white bg-red-600 hover:bg-red-700 rounded-lg transition disabled:opacity-50" title="Logout">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="flex items-center gap-6 mb-8">
      <div class="flex flex-col items-center">
        <img :src="computedThumbnail" alt="Avatar"
    ...
    <div v-else class="space-y-4">
      <input v-model="account.details.firstname" type="text" placeholder="First Name"
        class="w-full px-3 py-2 border border-gray-300 rounded-lg" />
      <input v-model="account.details.lastname" type="text" placeholder="Last Name"
        class="w-full px-3 py-2 border border-gray-300 rounded-lg" />
      <textarea v-model="account.details.description" placeholder="Description"
        class="w-full px-3 py-2 border border-gray-300 rounded-lg"></textarea>
      <input v-model="account.details.birthday" type="date"
        class="w-full px-3 py-2 border border-gray-300 rounded-lg" />
